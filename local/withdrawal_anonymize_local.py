...
LOG_FILE = ("/home/odoo/Documents/wibtec/PP-1231/withdrawal/withdrawal_update_log_file.csv")
...
try:
    ...
    cur.execute("SELECT id FROM withdrawal WHERE pmx_id = '' ORDER BY id LIMIT 2000")
    withdrawal_ids = [row[0] for row in cur.fetchall()]
    ...
    num = 0
    counter = 0
    for index in range(0, len(withdrawal_ids), batch_size):
        batch = withdrawal_ids[index : index + batch_size]
        counter = index
        for row in batch:
            try:
                num+=1
                logging.info(f"Processing : Row Number {num} : Withdrawal Record ID {row}")
                random_number = generate_random_unique_number()
                new_pmx_id = f"withdrawal{random_number}"
                ...
                WHERE
                    id = %s
                """
                cur.executemany(query, withdrawal_list)
                conn.commit()
                withdrawal_list = []
