...
LOG_FILE = ("/home/odoo/Documents/wibtec/PP-1231/balances/balances_update_log_file.csv")
...
try:
    ...
    cur.execute("SELECT id FROM balance WHERE pmx_id = '' ORDER BY id")
    balance_ids = [row[0] for row in cur.fetchall()]
    ...
    num = 0
    counter = 0
    for index in range(0, len(balance_ids), batch_size):
        batch = balance_ids[index : index + batch_size]
        counter = index
        for row in batch:
            try:
                num+=1
                logging.info(f"Processing : Row Number {num} : Balance Record ID {row}")
                random_number = generate_random_unique_number()
                new_pmx_id = f"balance{random_number}"
                ...
                WHERE
                    id = %s
                """
                cur.executemany(query, balance_list)
                conn.commit()
                balance_list = []
