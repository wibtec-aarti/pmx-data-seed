...
LOG_FILE = "/home/odoo/Documents/Wibtec/2086_seed_data/balance/balance_log.csv"
...
# Define directory 
DIR_PATH = "/home/odoo/Documents/Wibtec/2086_seed_data/balance/file_chunks/"
            ...
            for index in range(0, len(reader), batch_size):
                ...
                for row in batch:
                    try:
                        ...
                        logging.info(f"Processing file {file_name} : Row Number {num} : balance Record ID {current_row_id}")
                        cur.execute(f"SELECT * FROM balance WHERE id = {current_row_id}")
                        ...
                        if not existing_balance_record:
                            balance_vals = {
                                ...
                                # "balance_date": row["balance_date"] if row["balance_date"] else None, #(Dattime: wrong data)
                                # "fs_client_id": row["fs_client_id"] if row["fs_client_id"] else None, #(Blank: M2O)
                            }
                        print(balance_vals)
                        balance_data.append(balance_vals)
                        ...
                        if balance_data:
                            ...
                            print("insert_queryinsert_queryinsert_queryinsert_query")
                            balance_data = []
