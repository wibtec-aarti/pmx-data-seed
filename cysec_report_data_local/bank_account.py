...
LOG_FILE = "/home/odoo/Documents/Wibtec/2086_seed_data/bank_account/bank_account_log.csv"
...
# Define directory 
DIR_PATH = "/home/odoo/Documents/Wibtec/2086_seed_data/bank_account/file_chunks/"
            ...
            for index in range(0, len(reader), batch_size):
                ...
                for row in batch:
                    try:
                        ...
                        logging.info(f"Processing file {file_name} : Row Number {num} : Bank Account Record ID {current_row_id}")
                        cur.execute(f"SELECT * FROM res_partner_bank WHERE id = {current_row_id}")
                        existing_res_partner_bank_record = cur.fetchone()

                        if not existing_res_partner_bank_record:
                            bank_account_vals = {
                                "active": True,
                                "id": row["id"],
                                "acc_number": row["acc_number"] if row["acc_number"] else None,
                                "partner_id": row["partner_id"] if row["partner_id"] else None,
                                # "acc_holder_name": row["acc_holder_name"] if row["acc_holder_name"] else None, >>>>>>>> [Blank]
                                "bank_id": row["bank_id"] if row["bank_id"] else None, #(M2O)
                                "pmx_id": row["pmx_id"] if row["pmx_id"] else None,
                                # "client_pmx_id": row["client_pmx_id"] if row["client_pmx_id"] else None, >>>>>>>> [Blank]
                                # "is_client_account": row["is_client_account"] if row["is_client_account"] else None, >>>>>>>> [Blank]
                                # "is_firm_account": row["is_firm_account"] if row["is_firm_account"] else None, >>>>>>>> [Blank]
                            }
                        print(bank_account_vals)
                        bank_account_data.append(bank_account_vals)
                        if bank_account_data:
                            columns = bank_account_data[0].keys()
                            values = [tuple(item.get(column) for column in columns)for item in bank_account_data]
                            insert_query = sql.SQL("INSERT INTO res_partner_bank ({}) VALUES ({})").format(
